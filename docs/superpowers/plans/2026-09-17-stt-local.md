...
**Spec:** `docs/superpowers/specs/2026-09-17-stt-local-design.md`
...
- `pyproject.toml`: package metadata, runtime dependencies, test dependencies, and `stt-local` entry point.
- `src/stt_local/constants.py`: fixed model, sample rate, trigger, poll interval, and idle timeout values.
- `src/stt_local/config.py`: application-support paths and atomic persisted settings.
- `src/stt_local/processors.py`: custom processor discovery, creation, selection, validation, and execution.
- `src/stt_local/audio.py`: capture adapter and trailing-silence trim.
- `src/stt_local/model_worker.py`: worker protocol and lifecycle manager.
- `src/stt_local/coordinator.py`: state machine and full dictation workflow.
- `src/stt_local/output.py`: clipboard and synthetic paste adapter.
- `src/stt_local/sounds.py`: asynchronous start/stop sound adapter.
- `src/stt_local/settings.py`: AppKit processor settings controller.
- `src/stt_local/app.py`: rumps status app and dependency composition.
- `src/stt_local/__main__.py`: multiprocessing-safe entry point.
...
- Create: `.gitignore`
- Create: `src/stt_local/__init__.py`
- Create: `src/stt_local/constants.py`
- Create: `src/stt_local/config.py`
- Create: `src/stt_local/processors.py`
...
Expected: collection fails because `stt_local.config` does not exist.
...
Expected: collection fails because `stt_local.audio` does not exist.
...
Commit: `git add src/stt_local/audio.py tests/test_audio.py && git commit -m "feat: add microphone capture"`

### Task 3: Resident Whisper worker with deterministic unload

**Files:**
- Create: `src/stt_local/model_worker.py`
...
Expected: collection fails because `stt_local.model_worker` does not exist.
...
Commit: `git add src/stt_local/model_worker.py tests/test_model_worker.py && git commit -m "feat: manage resident whisper worker"`

### Task 4: Coordinator, sounds, and output

**Files:**
- Create: `src/stt_local/coordinator.py`
- Create: `src/stt_local/output.py`
- Create: `src/stt_local/sounds.py`
...
Expected: collection fails because `stt_local.coordinator` does not exist.
...
Commit: `git add src/stt_local/coordinator.py src/stt_local/output.py src/stt_local/sounds.py tests && git commit -m "feat: orchestrate dictation workflow"`

### Task 5: Menu-bar app and processor settings window

**Files:**
- Create: `src/stt_local/settings.py`
- Create: `src/stt_local/app.py`
- Create: `src/stt_local/__main__.py`
...
Commit: `git add src/stt_local/settings.py src/stt_local/app.py src/stt_local/__main__.py tests && git commit -m "feat: add menu bar interface"`
...
**Interfaces:**
- Produces: launch command `uv run stt-local`.
- Produces: per-user LaunchAgent label `com.asempruch.stt-local` with temporary logs at `/tmp/stt-local.stdout.log` and `/tmp/stt-local.stderr.log`.
